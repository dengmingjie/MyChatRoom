...
void Client::loopSelect (void) throw (ClientException)
{
    ...
    while (1)
    {
        ...
        FD_SET (m_sockfd, &rSet);  // 添加到可读事件集合

        // 5ms
        timeout.tv_sec = 0;      // 秒
        timeout.tv_usec = 5000;  // 毫秒

        int nReady = select (m_sockfd + 1, &rSet, NULL, NULL, &timeout);
...
void Client::heartBeatCheck (void) throw (ClientException)
{
    ...
    if (now - m_lastRecvHeartBeat < HEARTRATE)  // 检查服务器心率
    {
        return;  // 服务器心跳正常
    }
    else if (now - m_lastRecvHeartBeat > HEARTRATE && now - m_lastRecvHeartBeat < 2*HEARTRATE)
    {
        ++m_heartStopTimes;  // 服务器心跳停止次数+1
    }
    else
    {
        m_heartStopTimes += 2; // 服务器心跳停止次数+2
    }
    if (m_heartStopTimes > MAXSTOPTIME)  // 判断服务器心跳停止次数是否异常
    {
        // 服务器异常，抛异常
        throw SocketException (strerror (errno));
    }
...
{
    if (m_recvBuf.m_isHeaderFull)  // 判断报头是否接收完
	{
	...
	{
		int dataLen;
        if (ioctl (m_sockfd, FIONREAD, &dataLen) < 0)  // 判断底层buf是否收到报头
            return false;
        if (dataLen < 20)  // 未收到20个字节的报头
			return false;

        int ret = recv (m_sockfd, &m_recvBuf.m_recvBuf, 20, 0);  // 接收报头
...
void Client::logInDeal (void)
{
    ...
    memcpy (m_sendBuf, &header, 20);
    memcpy (m_sendBuf+20, userName, 20);
    memcpy (m_sendBuf+40, userPWD, 20);

    // 发送数据
    sendData (header.length);
}
/* 私聊指令处理 */
void Client::pChatDeal (void)
{
    ...
    cin >> msg_buf;
    memcpy (m_sendBuf, &header, 20);
    memcpy (m_sendBuf+20, msg_buf, 1024);
...
/* 发送数据 */
void Client::sendData (int length) throw (SendException)
{
    ssize_t ret;
    for (size_t len = length; len; len -= ret)
    {
        ret = send (m_sockfd, m_sendBuf + length - len, len, 0);
        if (ret < 0)  // 失败返回-1，errno被设为以下的某个值
        {
            errnos(); // 错误处理
            return;
