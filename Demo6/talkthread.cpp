...
void* CTalkThread::loopTalk (void) 
{
	...
	while (1) 
	{
        ...
        for (map<int, CUser*>::iterator it = m_userMap.begin(); it != m_userMap.end(); ++it)
        {
            ...
            // 判断是否有未发完的数据
            else if (!it->second->m_sendBufEmpty)
            {
                // 设置关联相应用户
        ...
        for (int i = 0; i < nReady; ++i)
        {
            ...
            if (events[i].events & EPOLLIN)
            {
                ...
                if (pUser->m_pRecvBuf == NULL)
                {
                    CRecvBuf* pRecvBuf = new CRecvBuf;
                    pUser->m_pRecvBuf = pRecvBuf;
                }
                if (recvData (pUser, pUser->m_pRecvBuf))  // 接收数据
                {
                    if (pUser->m_pRecvBuf->header.userID < 0)
                    {
                        // 若为logIn指令，将userID放到pHeader中
                        int userID = pUser->getUserID();
                        memcpy (&pUser->m_pRecvBuf->header.userID, &userID, sizeof (userID));
                    }
                    g_cmdQueue.pushCmd (&pUser->m_pRecvBuf->header);  // 向上抛出命令
                    pUser->m_pRecvBuf = NULL;
                }
            }
            // 判断是否有可写事件
